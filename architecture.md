...
Ground Karate is a client-side single-user application built with Next.js App Router. All pages are client components (`"use client"`) since the app relies on browser APIs (localStorage, TipTap editors). The Next.js framework provides routing, bundling, and static optimization.
...
- **Authentication**: Add a provider in `layout.tsx` and gate routes. The `userId` field on entries is already present.
