...
-- use request.jwt.claims->sub directly for user scoping.
...
ALTER TABLE "profiles"
  ALTER COLUMN "id" SET DEFAULT ((current_setting('request.jwt.claims', true)::json->>'sub'));
ALTER TABLE "journal_entries"
...
CREATE POLICY "profiles_select_own"
  ...
  FOR SELECT
  TO authenticated
  USING (((current_setting('request.jwt.claims', true)::json->>'sub') = ("id")::text));
...
CREATE POLICY "profiles_insert_own"
  ...
  FOR INSERT
  TO authenticated
  WITH CHECK (((current_setting('request.jwt.claims', true)::json->>'sub') = ("id")::text));
